...
## scanf
### scanf读入的规则：
**自动跳过所有前置空白**：`%d` `%f` `%lf` `%s` `%x` 等绝大多数格式符 它们会自动跳过开头所有的空格、回车、Tab，直到找到第一个非空白字符才开始读取。
```C
char str[10];
int arr[10];
scanf("%s",str);
scanf("%d",&arr[0]);
//输入       hello
//str里存的：hello，没有前面的空格,arr同理
```

**在碰到了第一个非空白字符之后，它后面的所有特殊字符都会被读入**
因为scanf读入你输入的东西，它不是在你输入完之后，它就直接复制了。它会先把你键盘敲的东西放在缓存区，直到这个程序结束，或者你按了回车了，他才会去把缓存里的东西再放到对应的地址处
```C
char arr[10];
for(int i=0;i<10;i++){
	int c;
...
```
这里如果不加一个对C的读取，我在输入了我的`arr [i]`之后，我按了一个回车，那我的下一个`arr [i]`里面就存回车了
标准解法：不用再加一个c。
用`scanf(" %c", &c);` 格式符前加一个空格，让 scanf 主动跳过所有前置空白。
### 各格式符的读取终止规则
所有格式符遇到终止条件就会停止读取，且**终止符会残留在输入缓冲区**，不会被读走：
- `%s`：遇到空白字符（空格 / 回车 / Tab）立刻停止，这就是你输入带空格的字符串失效的原因。
- `%d` / `%f` 等数值格式：遇到非数字、非符号的字符立刻停止，比如输入 `123abc`，`%d` 只会读走 `123`，`abc` 全部留在缓冲区。
- `%[^\n]` 扫描集：读到换行符停止，可以读取空格（整行读取）；但如果开头就是换行，会直接读取失败。

### scanf 有返回值，而且非常有用
`scanf` 的返回值是**成功匹配并赋值的参数个数**，读取到文件末尾返回 `EOF`。
- `scanf("%d%d",&a,&b)`
- 输入 `1 2` →  返回 `2` 
- 输入 `1 a` → 返回 `1`
- 直接输入字母 → 返回 `0`
- 按下 `Ctrl+Z`(Windows) / `Ctrl+D`(Linux) → 返回 `EOF`

> 编译器提示的 `ignoring return value of scanf` 警告，就是提醒你没有处理读取失败的情况。 经典用法：`while(scanf("%d", &n) != EOF)` 实现多组输入，算法题高频使用。

## printf
### printf 也有返回值(了解即可)
返回值是**成功输出的字符总数**（包括空格、换行符等所有字符）。
```C
int len = printf("abc\n");
// len 的值是 4（a、b、c、换行符共4个字符）
```
## 一些小点
